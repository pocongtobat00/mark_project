#!/bin/bash
mkdir clang
wget -c https://github.com/greenforce-project/clang-llvm/releases/download/04102023/greenforce-clang-18.0.0-04102023-1050.tar.zst -O - | tar -xz -C clang
...
export send="/root/kernel/ven/send"
export ARCH="arm64"
export CLANG_DIR="/root/kernel/18"
export CC="${CLANG_DIR}/bin/clang"
export LINKER="lld"
export compiler_name="Gang Kapak Clang 18 Compiler"
...
build_ksu() {
	...
	ZIPNAME=Mark_KSU-Kernel.zip
}

build_oc() {
	mkdir $ANYKERNEL_DIR/Mark/backup
	mv arch/arm64/boot/dts/vendor/qcom/kona-v2-gpu.dtsi $ANYKERNEL_DIR/Mark/backup/
	cp $ANYKERNEL_DIR/Mark/kona-v2-gpu.dtsi arch/arm64/boot/dts/vendor/qcom/kona-v2-gpu.dtsi
	chmod 755 arch/arm64/boot/dts/vendor/qcom/kona-v2-gpu.dtsi
}

restore_gpu_dtsi() {
	mv $ANYKERNEL_DIR/Mark/backup/kona-v2-gpu.dtsi arch/arm64/boot/dts/vendor/qcom/kona-v2-gpu.dtsi > /dev/null 2>&1 
	chmod 755 arch/arm64/boot/dts/vendor/qcom/kona-v2-gpu.dtsi
	rm -rf $ANYKERNEL_DIR/mark/backup
}
	...
	if [[ -f $KERNEL_DIR/out/arch/arm64/boot/Image.gz ]] && [[ -f $KERNEL_DIR/out/arch/arm64/boot/dtb.img ]] && [[ -f $KERNEL_DIR/out/arch/arm64/boot/dtbo.img ]]; then
		...
		if [[ $DEVICE == apollo ]]; then
			if [[ ! -d $ANYKERNEL_DIR/ramdisk ]]; then
				...
				cp -r $ANYKERNEL_DIR/Mark/overlay.d $ANYKERNEL_DIR/ramdisk/
			fi
			cp $ANYKERNEL_DIR/Mark/apollo $ANYKERNEL_DIR/anykernel.sh
		elif [[ $DEVICE == alioth ]]; then
			rm -rf $ANYKERNEL_DIR/ramdisk
			cp $ANYKERNEL_DIR/Mark/alioth $ANYKERNEL_DIR/anykernel.sh
		elif [[ $DEVICE == munch ]]; then
			rm -rf $ANYKERNEL_DIR/ramdisk
			cp $ANYKERNEL_DIR/Mark/munch $ANYKERNEL_DIR/anykernel.sh
		fi
		zip -r9 $ZIPNAME META-INF ramdisk tools anykernel.sh banner Image.gz dtb dtbo.img
		mv $ZIPNAME $KERNEL_DIR/
		echo ""
		echo "send zip"
		bash $KERNEL_DIR/send
...
build() {
	...
	clear
      # Post to CI channel
      # curl -s -X POST https://api.telegram.org/bot${TOKEN}/sendMessage -d text="Buckle up Mark  kernel build has started" -d chat_id=${chat_id} -d parse_mode=HTML
      # curl -s -X POST https://api.telegram.org/bot${TOKEN}/sendMessage -d text="Branch: <code>$(git rev-parse --abbrev-ref HEAD)</code>
      # Kernel Version : <code>'Mark Kernel One'</code>
      # Compiler Used : <code>'Gang Kapak 17'</code>
